...
Codex usa configuración TOML, no JSON con `mcpServers`. Añade esto a `~/.codex/config.toml` o a un `.codex/config.toml` con alcance de proyecto:
...
Añade esto a `opencode.json` u `opencode.jsonc`:
