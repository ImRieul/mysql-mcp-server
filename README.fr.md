...
Codex utilise une configuration TOML, pas le JSON `mcpServers`. Ajoutez ceci à `~/.codex/config.toml` ou à un `.codex/config.toml` limité au projet :
...
Ajoutez ceci à `opencode.json` ou `opencode.jsonc` :
